...
namespace morph
{
     ...
     */
    template <typename S, size_t N>
    std::ostream &operator<<( std::ostream &, const vec<S, N> & );
    ...
    struct vec : public std::array<S, N>
    {
        ...
        S length() const
        {
            ...
            {
                const S len =
                    static_cast<S>( std::round( std::sqrt( std::accumulate( this->begin(), this->end(), S{ 0 }, add_squared ) ) ) );
                return len;
            }
            else
            {
        ...
        vec<S, N> lengthen( const S dl ) const
        {
            ...
            S         newlen = this->length() + dl;
            if( newlen <= S{ 0 } )
            { // dl could be negative, so still need to test new length of vector
                v.zero();
            }
            else
            {
        ...
        template <typename _S = S>
        bool length_lessthan( const vec<_S, N> &rhs ) const
        {
