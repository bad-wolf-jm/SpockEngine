...
namespace morph
{
     ...
     */
    template <typename S, typename Al>
    std::ostream &operator<<( std::ostream &, const vvec<S, Al> & );
    ...
    struct vvec : public std::vector<S, Al>
    {
        ...
        vvec<float> zerocross( const wrapdata wrap = wrapdata::none ) const
        {
                ...
                for( int i = 1; i < n; ++i )
                {
                    if( ( *this )[i] == S{ 0 } )
                    {
                        ...
                        else
                        {
                            ...
                            // else if (lastval > S{0} && ((*this)[i+1] > S{0})) { dirn = S{0}; } // fn touches 0 from above, DOESN'T
                            // cross else if (lastval < S{0} && ((*this)[i+1] < S{0})) { dirn = S{0}; } // fn touches 0 from below,
                            // DOESN'T cross
                        }
                    }
            ...
            { // wrapdata::wrap
                ...
                if( !crossings.empty() && std::abs( crossings[0] ) > 1.0f )
                {
                    crossings.rotate();
                }
            }
        ...
        template <typename _S = S>
        void max_elementwise_inplace( const _S &val )
        {
            for( auto &i : *this )
            {
                i = std::max( i, val );
        ...
        template <typename _S = S>
        void min_elementwise_inplace( const _S &val )
        {
            for( auto &i : *this )
            {
                i = std::min( i, val );
        ...
        vvec<S, Al> operator*( const vvec<_S> &v ) const
        {
            if( v.size() != this->size() )
            {
                throw std::runtime_error(
                    "vvec::operator*: Hadamard product is defined here for vectors of same dimensionality only" );
            }
        ...
        vvec<S, Al> operator/( const vvec<_S> &v ) const
        {
            ...
            {
                throw std::runtime_error(
                    "vvec::operator*: Hadamard division is defined here for vectors of same dimensionality only" );
            }
            vvec<S, Al> rtn( this->size(), S{ 0 } );
            auto        vi       = v.begin();
