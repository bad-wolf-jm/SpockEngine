    ...
    class ColourMap
    {
      ...
      public:
        ...
        std::array<float, 3> convert( T _datum1, T _datum2, T _datum3 )
        {
            ...
            return this->type == ColourMapType::Trichrome ? ( this->trichrome( _datum1, _datum2, _datum3 ) )
                                                          : ( this->rgb( _datum1, _datum2, _datum3 ) );
        }
        ...
        std::array<float, 3> convert( T _datum )
        {
            ...
            if constexpr( std::is_same<std::decay_t<T>, double>::value == true || std::is_same<std::decay_t<T>, float>::value == true )
            {
                if( std::isnan( datum ) == true )
                {
                    c = ColourMap<T>::nanColour( this->type );
                    return c;
                }
            }
      ...
      private:
         ...
         */
        std::array<float, 3> monochrome( float datum ) { return ColourMap::hsv2rgb( this->hue, datum, 1.0f ); }
        ...
        std::array<float, 3> hsv_2d( T datum1, T datum2 )
        {
            ...
            return ColourMap::hsv2rgb( this->hue_reverse_direction ? ( 1.0f - phi_hue_f ) : phi_hue_f, static_cast<float>( r_sat ),
                                       this->val );
        }
        ...
        std::array<float, 3> rainbow( float datum ) { return ColourMap::hsv2rgb( datum, 1.0f, 1.0f ); }
        ...
        std::array<float, 3> twilight( float datum )
        {
            ...
            std::array<float, 3> c = { morph::cm_twilight[datum_i][0], morph::cm_twilight[datum_i][1],
                                       morph::cm_twilight[datum_i][2] };
            return c;
        }
    };
