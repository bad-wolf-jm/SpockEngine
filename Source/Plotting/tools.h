...
namespace morph
{
    ...
    struct to_lower
    {
        char operator()( const char c ) const { return tolower( c ); }
    };

    //! Allows use of transform and toupper() on strings with GNU compiler
    struct to_upper
    {
        char operator()( const char c ) const { return toupper( c ); }
    };
    ...
    class Tools
    {
      public:
         ...
         */
        static int stripTrailingSpaces( std::string &input ) { return Tools::stripTrailingChars( input ); }
         ...
         */
        static int stripLeadingSpaces( std::string &input ) { return Tools::stripLeadingChars( input ); }
        ...
        static int stripLeadingWhitespace( std::string &input )
        {
            ...
            while( pos < input.size() && ( ( c = input[pos] ) == ' ' || c == '\t' || c == '\n' || c == '\r' ) )
            {
                pos++;
            }
            input.erase( 0, pos );
         ...
         */
        static int searchReplace( const std::string &searchTerm, const std::string &replaceTerm, std::string &data,
        ...
        static void toLowerCase( std::string &str ) { std::transform( str.begin(), str.end(), str.begin(), morph::to_lower() ); }

        //! Convert str to upper case
        static void toUpperCase( std::string &str ) { std::transform( str.begin(), str.end(), str.begin(), morph::to_upper() ); }
        ...
        static void conditionAsXmlTag( std::string &str )
        {
            ...
            std::transform( firstThree.begin(), firstThree.end(), firstThree.begin(), morph::to_lower() );
            if( firstThree == "xml" )
            {
                // Prepend 'A'
                std::string newStr( "_" );
         ...
         */
        static std::vector<std::string> csvToVector( const std::string &csvList, const char separator = ',',
                                                     const bool ignoreTrailingEmptyVal = true );

        /*!
         * split csv into a list
         */
        static std::list<std::string> csvToList( const std::string &csvList, const char separator = ',' );
        /*!
         * split csv into a set
         */
        static std::set<std::string> csvToSet( const std::string &csvList, const char separator = ',' );

        /*!
         * Output a vector of strings as a csv string.
         */
        static std::string vectorToCsv( const std::vector<std::string> &vecList, const char separator = ',' );

        /*!
         * Output a list of strings as a csv string.
         */
        static std::string listToCsv( const std::list<std::string> &listList, const char separator = ',' );

        /*!
         * Output a set of strings as a csv string.
         */
        static std::string setToCsv( const std::set<std::string> &listList, const char separator = ',' );

        /*!
         * Split a comma-separated key/value pair list into a map.
         */
        static std::map<std::string, std::string> csvToMap( const std::string &csvList, const char relationship = '=',
        ...
        static void createDir( const std::string &path ) { std::filesystem::create_directories( path ); }
        ...
        static bool fileExists( const std::string &path )
        {
            ...
            if( S_ISREG( buf->st_mode ) || S_ISBLK( buf->st_mode ) || S_ISSOCK( buf->st_mode ) || S_ISFIFO( buf->st_mode ) ||
         ...
         */
        static void createDir( const std::string &path, const mode_t mode = 0775, const int uid = -1, const int gid = -1 )
        {
            if( path.empty() )
            {
                return;
            ...
            while( i != dirs.rend() )
            {
                ...
                if( rtn )
                {
                    ...
                    switch( e )
                    {
                    case EACCES: emsg << "Permission is denied"; break;
                    case EEXIST:
                        ...
                        continue;
                        break;
                    case EFAULT: emsg << "Bad address"; break;
                    case ELOOP: emsg << "Too many symlinks in " << prePath; break;
                    case ENAMETOOLONG: emsg << "File name (" << prePath << ") too long"; break;
                    case ENOENT: emsg << "Path '" << prePath << "' invalid (part or all of it)"; break;
                    case ENOMEM: emsg << "Out of kernel memory"; break;
                    case ENOSPC: emsg << "Out of storage space/quota exceeded."; break;
                    case ENOTDIR: emsg << "component of the path '" << prePath << "' is not a directory"; break;
                    case EPERM: emsg << "file system doesn't support directory creation"; break;
                    case EROFS: emsg << "path '" << prePath << "' refers to location on read only filesystem"; break;
        ...
        static void removeDir( const std::string &path )
        {
            ...
            if( rtn )
            {
                ...
                {
                case EACCES: emsg << "Permission is denied"; break;
                case EBUSY: emsg << "Path in use"; break;
                case EFAULT: emsg << "Bad address"; break;
                case EINVAL: emsg << "Path has . as last component"; break;
                case ELOOP: emsg << "Too many symlinks"; break;
                case ENAMETOOLONG: emsg << "File name too long"; break;
                case ENOENT: emsg << "Path invalid (part or all of it)"; break;
                case ENOMEM: emsg << "Out of kernel memory"; break;
                case ENOTDIR: emsg << "component of the path is not a directory"; break;
                case EPERM: emsg << "file system doesn't support directory creation"; break;
                case EROFS: emsg << "path refers to location on read only filesystem"; break;
                default: emsg << "unknown error"; break;
                }
                throw std::runtime_error( emsg.str() );
            }
        ...
        static void setPermissions( const std::string &filepath, const mode_t mode )
        {
            ...
            if( rtn )
            {
                ...
                case EPERM: emsg << "file system doesn't support directory creation"; break;
                case EROFS: emsg << "path refers to location on read only filesystem"; break;
                case EBADF: emsg << "file descriptor is not valid"; break;
                case EIO: emsg << "an i/o error occurred"; break;
                default: emsg << "unknown error"; break;
                }
                throw std::runtime_error( emsg.str() );
            }
         ...
         */
        static void setOwnership( const std::string &filepath, const int uid = -1, const int gid = -1 )
        {
...
#define COPYFILE_BUFFERSIZE_MM 32767 // MM: Minus Minus
        ...
        static void copyFile( const std::string &from, std::ostream &to )
        {
            ...
            {
                std::stringstream ee;
                ee << "Tools::copyFile(): FROM file '" << from << "' is not a regular file";
                throw std::runtime_error( ee.str() );
            }
         ...
         * \param truncated.
         */
        static void truncateFile( const std::string &original, const std::string &truncated, const unsigned int bytes )
        {
            std::ofstream out;
            ...
            {
                std::stringstream ee;
                ee << "Tools::truncateFile(): ORIGINAL file '" << original << "' is not a regular file";
                throw std::runtime_error( ee.str() );
            }
        ...
        static void unlinkFile( const std::string &fpath )
        {
            ...
            if( rtn )
            {
                ...
                {
                case EPERM:
                case EACCES: emsg = "Write access to '" + fpath + "' is not allowed due to permissions"; break;
                case EBUSY: emsg = "'" + fpath + "' cannot be removed as it is in use by another process"; break;
                case EFAULT: emsg = "'" + fpath + "' points outside your accessible address space"; break;
                case EIO: emsg = "I/O error occurred reading '" + fpath + "'"; break;
                case EISDIR: emsg = "'" + fpath + "' is a directory"; break;
                case ELOOP: emsg = "Too many symlinks encountered in '" + fpath + "'"; break;
                case ENAMETOOLONG: emsg = "'" + fpath + "' is too long a name"; break;
                case ENOENT: emsg = "'" + fpath + "' does not exist or is a dangling symlink"; break;
                case ENOMEM: emsg = "In sufficient kernel memory to open '" + fpath + "'"; break;
                case ENOTDIR: emsg = "'" + fpath + "' contains a component that is not a directory"; break;
                case EROFS: emsg = "'" + fpath + "' is on a read-only filesystem"; break;
                default: emsg = "Unknown error unlinking file '" + fpath + "'"; break;
                }
         ...
         * contain filePart.
         */
        static void clearoutDir( const std::string &dirPath, const unsigned int olderThanSeconds = 0,
         ...
         */
        static void readDirectoryTree( std::vector<std::string> &vec, const std::string &baseDirPath, const std::string &subDirPath,
            ...
            while( ( ep = readdir( d ) ) )
            {
                ...
                else
                {
                    ...
                    if( olderThanSeconds > 0 )
                    {
                        ...
                        if( static_cast<unsigned int>( time( NULL ) ) - buf.st_mtime <= olderThanSeconds )
                        {
         ...
         * than olderThanSeconds will be returned.
         */
        static void readDirectoryTree( std::vector<std::string> &vec, const std::string &dirPath,
                                       const unsigned int olderThanSeconds = 0 )
        {
         ...
         */
        static void readDirectoryDirs( std::set<std::string> &dset, const std::string &dirPath )
        {
            DIR           *d;
            struct dirent *ep;
            size_t         entry_len = 0;
         ...
         * subDirPath should have NO INITIAL '/' character.
         */
        static void readDirectoryEmptyDirs( std::set<std::string> &dset, const std::string &baseDirPath,
         ...
         */
        static void removeEmptySubDirs( std::set<std::string> &dset, const std::string &baseDirPath, const std::string &subDir = "" )
        {
            DIR           *d;
            struct dirent *ep;
            size_t         entry_len = 0;
        ...
        static std::string monthStr( const int month, const bool shortFormat = false )
        {
            ...
            if( shortFormat == true )
            {
                switch( month )
                {
                case 1: rtn = "Jan"; break;
                case 2: rtn = "Feb"; break;
                case 3: rtn = "Mar"; break;
                case 4: rtn = "Apr"; break;
                case 5: rtn = "May"; break;
                case 6: rtn = "Jun"; break;
                case 7: rtn = "Jul"; break;
                case 8: rtn = "Aug"; break;
                case 9: rtn = "Sep"; break;
                case 10: rtn = "Oct"; break;
                case 11: rtn = "Nov"; break;
                case 12: rtn = "Dec"; break;
                default: rtn = "unk"; break;
                }
            }
            else
            {
                switch( month )
                {
                case 1: rtn = "January"; break;
                case 2: rtn = "February"; break;
                case 3: rtn = "March"; break;
                case 4: rtn = "April"; break;
                case 5: rtn = "May"; break;
                case 6: rtn = "June"; break;
                case 7: rtn = "July"; break;
                case 8: rtn = "August"; break;
         ...
         */
        static std::string numToDateTime( const time_t epochSeconds, const char dateSeparator = '\0', const char timeSeparator = '\0',
         ...
         */
#ifndef __WIN__
        static std::string filenameTimestamp() { return morph::Tools::numToDateTime( time( NULL ), '\0', '\0', '_' ); }
#endif
        /*!
        ...
        static std::vector<ST> splitStringWithEncs( const ST &s, const ST &separatorChars = ST( ";, " ),
 ...
 */
template <typename ST>
std::vector<ST> morph::Tools::splitStringWithEncs( const ST &s, const ST &separatorChars, const ST &enclosureChars,
                                                   const typename ST::value_type &escapeChar ) // or '\0'
{
    ...
    while( a < sz )
    {
        ...
        if( range != ST::npos )
        { // end of field was not end of string
            if( nextIsEnc )
            {
