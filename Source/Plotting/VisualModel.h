    ...
    class VisualModel
    {
      ...
      public:
        ...
        virtual void render()
        {
            ...
            if( !this->indices.empty() )
            {
                ...
                if( loc_a != -1 )
                {
                    glUniform1f( loc_a, this->alpha );
                }

                GLint loc_v = glGetUniformLocation( this->shaders.gprog, static_cast<const GLchar *>( "v_matrix" ) );
                if( loc_v != -1 )
                {
                    glUniformMatrix4fv( loc_v, 1, GL_FALSE, this->scenematrix.mat.data() );
                }
                ...
                if( loc_m != -1 )
                {
                    glUniformMatrix4fv( loc_m, 1, GL_FALSE, ( this->model_scaling * this->viewmatrix ).mat.data() );
                }
      ...
      public:
        ...
        morph::TextGeometry addLabel( const std::string &_text, const morph::vec<float, 3> &_toffset,
                                      ...
                                      const morph::VisualFont _font = morph::VisualFont::DVSans, const float _fontsize = 0.05,
        ...
        morph::TextGeometry addLabel( const std::string &_text, const morph::vec<float, 3> &_toffset, morph::VisualTextModel *&tm,
                                      const std::array<float, 3> &_tcolour = morph::colour::black,
                                      const morph::VisualFont _font = morph::VisualFont::DVSans, const float _fontsize = 0.05,
                                      const int _fontres = 24 )
        {
      ...
      protected:
        ...
        // The max and min values in the next 8 attriubutes are only computed if gltf files are going to be output by
        // Visual::safegltf()

        //! Max values of 0th, 1st and 2nd coordinates in vertexPositions
        morph::vec<float, 3> vpos_maxes = { std::numeric_limits<float>::lowest(), std::numeric_limits<float>::lowest(),
                                            std::numeric_limits<float>::lowest() };
        //! Min values in vertexPositions
        morph::vec<float, 3> vpos_mins   = { std::numeric_limits<float>::max(), std::numeric_limits<float>::max(),
                                             std::numeric_limits<float>::max() };
        morph::vec<float, 3> vcol_maxes  = { std::numeric_limits<float>::lowest(), std::numeric_limits<float>::lowest(),
                                             std::numeric_limits<float>::lowest() };
        morph::vec<float, 3> vcol_mins   = { std::numeric_limits<float>::max(), std::numeric_limits<float>::max(),
                                             std::numeric_limits<float>::max() };
        morph::vec<float, 3> vnorm_maxes = { std::numeric_limits<float>::lowest(), std::numeric_limits<float>::lowest(),
                                             std::numeric_limits<float>::lowest() };
        morph::vec<float, 3> vnorm_mins  = { std::numeric_limits<float>::max(), std::numeric_limits<float>::max(),
                                             std::numeric_limits<float>::max() };
         ...
         */
        void computeTube( VBOint &idx, vec<float> start, vec<float> end, std::array<float, 3> colStart, std::array<float, 3> colEnd,
         ...
         */
        void computeTube( VBOint &idx, vec<float> start, vec<float> end, vec<float> _ux, vec<float> _uy, std::array<float, 3> colStart,
                          std::array<float, 3> colEnd, float r = 1.0f, int segments = 12, float rotation = 0.0f )
        {
        ...
        void computeFlatQuad( VBOint &idx, vec<float> c1, vec<float> c2, vec<float> c3, vec<float> c4, std::array<float, 3> col )
        {
            // is the face normal
            vec<float> u1 = c1 - c2;
            vec<float> u2 = c2 - c3;
         ...
         * tube. Useful if this is to be a short tube used as a graph marker.
         */
        void computeFlatPoly( VBOint &idx, vec<float> vstart, vec<float> _ux, vec<float> _uy, std::array<float, 3> col, float r = 1.0f,
                              int segments = 12, float rotation = 0.0f )
        {
            // v is a face normal
            vec<float> v = _uy.cross( _ux );
            v.renormalize();
         ...
         */
        void computeRing( VBOint &idx, vec<float> ro, std::array<float, 3> rc, float r = 1.0f, float t = 0.1f, int segments = 12 )
        {
            for( int j = 0; j < segments; j++ )
            {
         ...
         */
        void computeSphere( VBOint &idx, vec<float> so, std::array<float, 3> sc, float r = 1.0f, int rings = 10, int segments = 12 )
        {
         ...
         * \param segments Number of segments used to render the sphere
         */
        void computeSphere( VBOint &idx, vec<float> so, std::array<float, 3> sc, std::array<float, 3> sc2, float r = 1.0f,
         ...
         * \param segments Number of segments used to render the tube
         */
        void computeCone( VBOint &idx, vec<float> centre, vec<float> tip, float ringoffset, std::array<float, 3> col, float r = 1.0f,
                          int segments = 12 )
        {
        ...
        void computeLine( VBOint &idx, vec<float> start, vec<float> end, vec<float> _uz, std::array<float, 3> col, float w = 0.1f,
                          float thickness = 0.01f, float shorten = 0.0f )
        {
         ...
         */
        void computeLine( VBOint &idx, vec<float> start, vec<float> end, vec<float> _uz, std::array<float, 3> colStart,
                          std::array<float, 3> colEnd, float w = 0.1f, float thickness = 0.01f, float shorten = 0.0f )
        {
        ...
        void computeFlatLine( VBOint &idx, vec<float> start, vec<float> end, vec<float> uz, std::array<float, 3> col, float w = 0.1f,
                              float shorten = 0.0f )
        {
            // The vector from start to end defines direction of the tube
            vec<float> vstart = start;
        ...
        // out a semi-circle).
        void computeFlatLineRnd( VBOint &idx, vec<float> start, vec<float> end, vec<float> uz, std::array<float, 3> col,
                                 float w = 0.1f, float shorten = 0.0f, bool startcaps = true, bool endcaps = true )
        {
            ...
            if( startcaps )
            { // prolly startcaps, for flexibility
                VBOint topcap = idx;
                for( int j = 0; j < segments; j++ )
                {
                    int inc1 = 1 + j;
        ...
        void computeFlatLine( VBOint &idx, vec<float> start, vec<float> end, vec<float> prev, vec<float> next, vec<float> uz,
                              std::array<float, 3> col, float w = 0.1f )
        {
            // The vector from start to end defines direction of the tube
            vec<float> vstart = start;
            vec<float> vend   = end;
        ...
        void computeFlatLineP( VBOint &idx, vec<float> start, vec<float> end, vec<float> prev, vec<float> uz, std::array<float, 3> col,
                               float w = 0.1f )
        {
            // The vector from start to end defines direction of the tube
            vec<float> vstart = start;
        ...
        void computeFlatLineN( VBOint &idx, vec<float> start, vec<float> end, vec<float> next, vec<float> uz, std::array<float, 3> col,
                               float w = 0.1f )
        {
            // The vector from start to end defines direction of the tube
            vec<float> vstart = start;
        ...
        void computeFlatDashedLine( VBOint &idx, vec<float> start, vec<float> end, vec<float> uz, std::array<float, 3> col,
                                    float w = 0.1f, float shorten = 0.0f, float dashlen = 0.1f, float gapprop = 0.3f )
        {
            if( dashlen == 0.0f )
            {
                return;
