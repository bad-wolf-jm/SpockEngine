...
namespace morph
{
    ...
    class ScaleImplBase
    {
      public:
         ...
         */
        template <template <typename, typename> typename Container, typename TT = T, typename Allocator = std::allocator<TT>,
                  template <typename, typename> typename OContainer = Container, typename ST = S,
         ...
         */
        template <template <typename, typename> typename OContainer, typename ST = S, typename OAllocator = std::allocator<ST>,
                  template <typename, typename> typename Container = OContainer, typename TT = T,
        ...
        virtual S transform_one( const T &datum ) const
        {
            ...
            if( params.size() != 2 )
            {
                throw std::runtime_error(
                    "For linear scaling of ND vector lengths, need 2 params (set do_autoscale or call setParams())" );
            }
            S    rtn( datum );
    ...
    class ScaleImpl<1, T, S> : public ScaleImplBase<T, S>
    {
      ...
      private:
        ...
        S transform_one_linear( const T &datum ) const { return ( datum * this->params[0] + this->params[1] ); }

        //! Log transform for scalar type
        S transform_one_log( const T &datum ) const { return ( transform_one_linear( std::log( datum ) ) ); }

        //! The inverse linear transform; x = (y-c)/m
        T inverse_one_linear( const S &datum ) const { return ( ( datum - this->params[1] ) / this->params[0] ); }
