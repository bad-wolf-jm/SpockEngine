...
namespace SE::Core::Primitives
{
    ...
    VertexBufferData CreatePlane( const math::ivec2 &subdivisions )
    {
        ...
        l_VertexData.Vertices = vec_t<SE::Core::VertexData>( vertexCount.x * vertexCount.y );
        l_VertexData.Indices  = vec_t<uint32_t>( (uint32_t)faceCount.x * faceCount.y * 6 );

        /* Indices */
        // vec_t<uint32_t> indexData{(uint32_t) faceCount.x * faceCount.y * 6};
        std::size_t i = 0;
    ...
    WireframeVertexBufferData CreateWireframeGrid( const math::ivec2 &subdivisions )
    {
        ...
        l_VertexData.Vertices = vec_t<math::vec3>( vertexCount.x * vertexCount.y );
        l_VertexData.Indices =
            vec_t<uint32_t>( ( vertexCount.y * ( vertexCount.x - 1 ) * 2 + vertexCount.x * ( vertexCount.y - 1 ) * 2 ) );
