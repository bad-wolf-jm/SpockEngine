...
namespace LTSE::Graphics::Internal
{
    ...
    sVkDescriptorSetObject::sVkDescriptorSetObject( Ref<VkContext> aContext, VkDescriptorPool aDescriptorPool, VkDescriptorSet aDescriporSet )
        : mContext{ aContext }
    ...
    void sVkDescriptorSetObject::Write( sBufferBindInfo aBuffers )
    {
        ...
        {
            lWriteDSOps.descriptorType = aBuffers.mDynamicOffset ? VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC : static_cast<VkDescriptorType>( eDescriptorType::STORAGE_BUFFER );
        }
        else if( aBuffers.mType == eBufferBindType::UNIFORM_BUFFER )
        {
            lWriteDSOps.descriptorType = aBuffers.mDynamicOffset ? VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC : static_cast<VkDescriptorType>( eDescriptorType::UNIFORM_BUFFER );
        }
        else
        {
    ...
    {
        return LTSE::Core::New<sVkDescriptorSetObject>( mContext, mVkObject, mContext->AllocateDescriptorSet( mVkObject, aLayout->mVkObject, aDescriptorCount ) );
    }
    ...
    sBufferLayoutElement::sBufferLayoutElement( const std::string &aName, eBufferDataType aType, uint32_t aBinding, uint32_t aLocation )
        : mName( aName )
        , mType( aType )
        , mBinding( aBinding )
        , mLocation( aLocation )
    ...
    void sBufferLayout::Compile( uint32_t aBinding, VkVertexInputBindingDescription &aBindingDesc, std::vector<VkVertexInputAttributeDescription> &aAttributes, bool aInstanced )
    {
    ...
    sVkPipelineObject::sVkPipelineObject( Ref<VkContext> aContext, uint8_t aSampleCount, sBufferLayout aVertexBufferLayout, sBufferLayout aInstanceBufferLayout,
                                          ePrimitiveTopology aTopology, eFaceCulling aCullMode, float aLineWidth, sDepthTesting aDepthTest, sBlending aBlending,
                                          std::vector<sShader> aShaderStages, Ref<sVkPipelineLayoutObject> aPipelineLayout, Ref<sVkRenderPassObject> aRenderPass )
        : mContext{ aContext }
    {
