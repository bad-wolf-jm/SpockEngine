...
namespace LTSE::Core::UI
{
    ...
    void CanvasView::DrawAxis( float a_ViewSize, math::vec2 a_AxisBound, const math::vec2 &from, const math::vec2 &to, float labelAlignment, float sign )
    {
        ...
        if( math::dot( direction, direction ) < FLT_EPSILON )
            return;
        ...
        for( auto d = 0.0f; d <= l_AxisLength * l_Scale; d += MinorUnit * l_Scale, p += direction * MinorUnit / l_Scale )
            drawList->AddLine( to_ImVec( p - normal * minorSize ), to_ImVec( p + normal * minorSize ), IM_COL32( 128, 128, 128, 255 ) );
    ...
    void CanvasView::DrawGrid()
    {
        ...
        for( auto d = XAxisBounds.x; d <= XAxisBounds.y; d += MinorUnit )
            l_DrawList->AddLine( ImVec2{ d * l_Scale.x, viewRect.Min.y }, ImVec2{ d * l_Scale.x, viewRect.Max.y }, IM_COL32( 255, 255, 255, 15 ) );

        for( auto d = YAxisBounds.x; d <= YAxisBounds.y; d += MinorUnit )
            l_DrawList->AddLine( ImVec2{ viewRect.Min.x, -d * l_Scale.y }, ImVec2{ viewRect.Max.x, -d * l_Scale.y }, IM_COL32( 255, 255, 255, 15 ) );
    ...
    {
        return math::vec2{ ( viewRect.Max.x - viewRect.Min.x ) / ( XAxisBounds.y - XAxisBounds.x ), ( viewRect.Max.y - viewRect.Min.y ) / ( YAxisBounds.y - YAxisBounds.x ) };
    }
    ...
    void CanvasView::AddRect( math::vec2 a_TopLeft, math::vec2 a_BottomRight, math::vec4 a_Color, float a_Thickness )
    {
        ...
        l_DrawList->AddRect( ImVec2{ a_TopLeft.x, -a_TopLeft.y }, ImVec2{ a_BottomRight.x, -a_BottomRight.y },
                             IM_COL32( static_cast<uint8_t>( a_Color.x * 255.0f ), static_cast<uint8_t>( a_Color.y * 255.0f ), static_cast<uint8_t>( a_Color.z * 255.0f ),
                                       static_cast<uint8_t>( a_Color.w * 255.0f ) ),
                             0, 0, a_Thickness );
    }
    ...
    void CanvasView::AddImageRect( ImageHandle a_Image, math::vec2 a_TopLeft, math::vec2 a_BottomRight, math::vec2 a_TopLeftUV, math::vec2 a_BottomRightUV )
    {
        auto l_DrawList    = ImGui::GetWindowDrawList();
        math::vec2 l_Scale = GetScale();
        a_TopLeft          = a_TopLeft * l_Scale;
        a_BottomRight      = a_BottomRight * l_Scale;
        l_DrawList->AddImage( (ImTextureID)a_Image.Handle->GetVkDescriptorSet(), ImVec2{ a_TopLeft.x, -a_TopLeft.y }, ImVec2{ a_BottomRight.x, -a_BottomRight.y },
                              ImVec2{ a_TopLeftUV.x, a_TopLeftUV.y }, ImVec2{ a_BottomRightUV.x, a_BottomRightUV.y },
                              IM_COL32( static_cast<uint8_t>( 255.0f ), static_cast<uint8_t>( 255.0f ), static_cast<uint8_t>( 255.0f ), static_cast<uint8_t>( 255.0f ) ) );
    }
    ...
    void CanvasView::Display( std::function<void( CanvasView * )> a_DisplayElements )
    {
        ...
        if( canvas.Begin( "##mycanvas", ImVec2( canvas_sz.x, 0.0f ) ) )
        {
            ...
            if( viewRect.Max.x > 0.0f )
                DrawAxis( ( l_ViewRect.Max.x - l_ViewRect.Min.x ), XAxisBounds, math::vec2( 0.0f, 0.0f ), math::vec2( viewRect.Max.x, 0.0f ), .5f );
            if( viewRect.Min.x < 0.0f )
                DrawAxis( ( l_ViewRect.Max.x - l_ViewRect.Min.x ), XAxisBounds, math::vec2( 0.0f, 0.0f ), math::vec2( viewRect.Min.x, 0.0f ), .5f, -1.0f );
            if( viewRect.Max.y > 0.0f )
                DrawAxis( ( l_ViewRect.Max.y - l_ViewRect.Min.y ), YAxisBounds, math::vec2( 0.0f, 0.0f ), math::vec2( 0.0f, viewRect.Max.y ), .5f, -1.0f );
            if( viewRect.Min.y < 0.0f )
                DrawAxis( ( l_ViewRect.Max.y - l_ViewRect.Min.y ), YAxisBounds, math::vec2( 0.0f, 0.0f ), math::vec2( 0.0f, viewRect.Min.y ), .5f );
