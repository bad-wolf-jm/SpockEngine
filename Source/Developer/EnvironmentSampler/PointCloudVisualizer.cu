...
namespace LTSE::SensorModel::Dev
{
    ...
    extern "C" __device__ LidarCartesianSamplePoint LidarDataToCartesian( math::mat3 a_PointCloudRotation, HitRecord a_LidarReturnPoints, bool a_InvertZAxis )
    {
    ...
    extern "C" __global__ void __kernel__FillParticleBuffer( math::mat3 a_PointCloudRotation, math::vec3 a_PointCloudOrigin, MultiTensor a_LidarReturnPoints,
                                                             GPUExternalMemory a_LidarParticle, bool a_LogScale, bool a_HighlightFOV, bool a_InvertZAxis, float aResolution )
    {
        int l_InputArrayIdx = blockIdx.x * blockDim.x + threadIdx.x;
        if( l_InputArrayIdx >= a_LidarReturnPoints.SizeAs<HitRecord>() )
            return;

        HitRecord *lHitRecords   = a_LidarReturnPoints.DataAs<HitRecord>();
        Particle *lLidarParticle = a_LidarParticle.DataAs<Particle>();

        LidarCartesianSamplePoint l_CartesianPoint = LidarDataToCartesian( a_PointCloudRotation, lHitRecords[l_InputArrayIdx], a_InvertZAxis );
        ...
        lLidarParticle[l_InputArrayIdx].PositionAndSize =
            math::vec4( a_PointCloudOrigin + l_CartesianPoint.Direction * l_CartesianPoint.Distance, glm::tan( aResolution / 2.0f ) * l_CartesianPoint.Distance * 2.0f );
        lLidarParticle[l_InputArrayIdx].Color = math::vec4( l_Color, 0.95 );
    }

    void PointCloudVisualizer::Visualize( math::mat4 a_PointCloudTransform, MultiTensor &a_LidarReturnPoints, GPUExternalMemory &a_Particles )
    {
        ...
        a_Particles.Zero();
        __kernel__FillParticleBuffer<<<l_GridDim, l_BlockDim>>>( math::NormalMatrix( a_PointCloudTransform ), math::Translation( a_PointCloudTransform ), a_LidarReturnPoints,
                                                                 a_Particles, LogScale, HighlightFlashFOV, InvertZAxis, Resolution * RADIANS );
