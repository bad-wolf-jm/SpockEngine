...
//  delegate                - Output parameter, the function stores a native callable function pointer to the delegate at the specified
//  address
//
// Returns:
//  HRESULT indicating status of the operation. S_OK if the assembly was successfully executed
...
//  HRESULT indicating status of the operation. S_OK if the assembly was successfully executed
//
CORECLR_HOSTING_API( coreclr_execute_assembly, void *hostHandle, unsigned int domainId, int argc, const char **argv,
...
static string_t build_tpa( const string_t &core_root, const string_t &core_libraries )
{
    ...
    for( const char_t *const *curr_ext = tpa_extensions; *curr_ext != nullptr; ++curr_ext )
    {
        ...
        for( const string_t &dir : { core_libraries, core_root } )
        {
            ...
            string_t tmp = pal::build_file_list( dir, ext,
                                                 [&]( const char_t *file )
                                                 {
...
//   public:
//     logger_t( const char *exePath, int propertyCount, const char **propertyKeys, const char **propertyValues,
...
static int run( const configuration &config )
{
    ...
    if( config.wait_to_debug ) wait_for_debugger();
    ...
    string_t core_root = !config.clr_path.empty() ? config.clr_path : pal::getenv( envvar::coreRoot );
...

    {
        ...
        if( !mock_hostpolicy.empty() && !pal::try_load_hostpolicy( mock_hostpolicy ) )
        {
            return -1;
        }
    }
...
Ref<argparse::ArgumentParser> ParseCommandLine( int argc, char **argv )
{
    ...
    // clang-format off
    lProgramArguments->add_argument( "-r", "--runtime" )
        .help( "Override the path to the coreclr library" )
        .default_value( std::string{ "" } );

    lProgramArguments->add_argument( "-p", "--program" )
        .help( "Specify the program to run" )
        .default_value( std::string{ "" } );
    // clang-format on
...
static bool parse_args( const int argc, const char_t *argv[], configuration &config )
{
    ...
    for( int i = 1; i < argc; i++ )
    {
        ...
        else if( ( pal::strcmp( option, W( "?" ) ) == 0 ||
                   ( pal::strcmp( option, W( "h" ) ) == 0 || ( pal::strcmp( option, W( "help" ) ) == 0 ) ) ) )
        {
            display_usage();
            break;
        }
