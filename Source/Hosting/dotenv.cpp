...
namespace
{
    ...
    bool read_var_value( std::istream &file, std::function<std::string( std::string )> substitution_lookup,
                         std::string &var_value_out )
    {
        ...
        while( !file.eof() )
        {
            ...
            else if( is_quoted && next == '\\' )
            {
                ...
                if( quote_char == '\'' )
                {
                    ...
                    {
                    case '\\':
                    case '\'': var_value.push_back( (char)escaped ); break;
                ...
                else
                {
                    ...
                    case '\'':
                    case '"': var_value.push_back( (char)escaped ); break;
                    case 'a': var_value.push_back( '\a' ); break;
                    case 'b': var_value.push_back( '\b' ); break;
                    case 'f': var_value.push_back( '\f' ); break;
                    case 'n': var_value.push_back( '\n' ); break;
                    case 'r': var_value.push_back( '\r' ); break;
                    case 't': var_value.push_back( '\t' ); break;
                    case 'v': var_value.push_back( '\v' ); break;
                    default: return false;
                    }
                }
            }
...
#define THROW_IF_FALSE( stmt ) \
    if( !( stmt ) ) throw W( #stmt );
