...
namespace LTSE::Core::UI
{
    ...
    template <typename _Ty> class ComboBox
    {
      public:
        std::string ID                  = "";
    ...
    template <typename T> static bool DragFloat( const std::string label, T &value, int label_width, float step, float min, float max )
    {
        char buf[128];
    ...
    template <typename T> static bool DragUInt( const std::string label, T &value, int label_width, float step, uint32_t min, uint32_t max )
    {
        char buf[128];
        ImFormatString( buf, IM_ARRAYSIZE( buf ), "##%s", label.c_str() );
    ...
    bool VectorComponentEditor( const std::string &a_Label, uint32_t ID, math::vec2 &values, float a_XMin, float a_XMax, float a_XStep, float a_YMin, float a_YMax, float a_YStep,
                                float a_ResetValue, float a_ColumnWidth );
