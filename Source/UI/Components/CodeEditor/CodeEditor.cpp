...
namespace SE::Core
{
    ...
    UICodeEditor::Coordinates UICodeEditor::GetActualCursorCoordinates() const
    {
        return SanitizeCoordinates( mState.mCursorPosition );
    }

    UICodeEditor::Coordinates UICodeEditor::SanitizeCoordinates( const Coordinates &aValue ) const
    {
    ...
    void UICodeEditor::Advance( Coordinates &aCoordinates ) const
    {
    ...
    void UICodeEditor::DeleteRange( const Coordinates &aStart, const Coordinates &aEnd )
    {
        assert( aEnd >= aStart );
        assert( !mReadOnly );
    ...
    int UICodeEditor::InsertTextAt( Coordinates & /* inout */ aWhere, const char *aValue )
    {
        assert( !mReadOnly );
    ...
    void UICodeEditor::AddUndo( UndoRecord &aValue )
    {
        assert( !mReadOnly );
    ...
    UICodeEditor::Coordinates UICodeEditor::ScreenPosToCoordinates( const ImVec2 &aPosition ) const
    {
    ...
    void UICodeEditor::RemoveLine( int aStart, int aEnd )
    {
        assert( !mReadOnly );
        assert( aEnd >= aStart );
    ...
    void UICodeEditor::RemoveLine( int aIndex )
    {
        assert( !mReadOnly );
    ...
    UICodeEditor::Line &UICodeEditor::InsertLine( int aIndex )
    {
        assert( !mReadOnly );
    ...
    string_t UICodeEditor::GetWordUnderCursor() const
    {
        auto c = GetCursorPosition();
        return GetWordAt( c );
    }

    string_t UICodeEditor::GetWordAt( const Coordinates &aCoords ) const
    {
    ...
    void UICodeEditor::HandleKeyboardInputs()
    {
    ...
    void UICodeEditor::HandleMouseInputs()
    {
        ImGuiIO &io    = ImGui::GetIO();
        auto     shift = io.KeyShift;
        auto     ctrl  = io.ConfigMacOSXBehaviors ? io.KeySuper : io.KeyCtrl;
    ...
    void UICodeEditor::Render( const char *aTitle, const ImVec2 &aSize, bool aBorder )
    {
        mWithinRender          = true;
        mTextChanged           = false;
        mCursorPositionChanged = false;
    ...
    void UICodeEditor::SetText( const string_t &aText )
    {
        mLines.clear();
    ...
    void UICodeEditor::EnterCharacter( ImWchar aChar, bool aShift )
    {
        assert( !mReadOnly );
        ...
        if( HasSelection() )
        {
            if( aChar == '\t' && mState.mSelectionStart.mLine != mState.mSelectionEnd.mLine )
            {
                ...
                for( int i = start.mLine; i <= end.mLine; i++ )
                {
                    ...
                    {
                        line.insert( line.begin(), Glyph( '\t', UICodeEditor::PaletteIndex::Background ) );
                        modified = true;
                    }
                }
    ...
    void UICodeEditor::SetSelection( const Coordinates &aStart, const Coordinates &aEnd, SelectionMode aMode )
    {
        ...
        case UICodeEditor::SelectionMode::Line:
        {
    ...
    void UICodeEditor::DeleteSelection()
    {
    ...
    void UICodeEditor::MoveUp( int aAmount, bool aSelect )
    {
    ...
    void UICodeEditor::MoveDown( int aAmount, bool aSelect )
    {
    ...
    void UICodeEditor::MoveRight( int aAmount, bool aSelect, bool aWordMode )
    {
        auto oldPos = mState.mCursorPosition;
    ...
    void UICodeEditor::MoveTop( bool aSelect )
    {
        auto oldPos = mState.mCursorPosition;
    ...
    void UICodeEditor::UICodeEditor::MoveBottom( bool aSelect )
    {
    ...
    void UICodeEditor::MoveHome( bool aSelect )
    {
        auto oldPos = mState.mCursorPosition;
    ...
    void UICodeEditor::MoveEnd( bool aSelect )
    {
        auto oldPos = mState.mCursorPosition;
    ...
    void UICodeEditor::SelectWordUnderCursor()
    {
        auto c = GetCursorPosition();
...
#define rgb( r, g, b ) IM_COL32( r, g, b, 255 )
    ...
    const UICodeEditor::Palette &UICodeEditor::GetLightPalette()
    {
        const static Palette p = { {
            0xff7f7f7f, // None
            0xffff0c06, // Keyword
    ...
    const UICodeEditor::Palette &UICodeEditor::GetRetroBluePalette()
    {
        const static Palette p = { {
            0xff00ffff, // None
            0xffffff00, // Keyword
    ...
    float UICodeEditor::TextDistanceToLineStart( const Coordinates &aFrom ) const
    {
    ...
    void UICodeEditor::EnsureCursorVisible()
    {
        if( !mWithinRender )
        {
    ...
    UICodeEditor::UndoRecord::UndoRecord( const string_t &aAdded, const UICodeEditor::Coordinates aAddedStart,
                                          const UICodeEditor::Coordinates aAddedEnd, const string_t &aRemoved,
                                          const UICodeEditor::Coordinates aRemovedStart, const UICodeEditor::Coordinates aRemovedEnd,
    ...
    void UICodeEditor::UndoRecord::Undo( UICodeEditor *aEditor )
    {
        if( !mAdded.empty() )
        {
    ...
    void UICodeEditor::UndoRecord::Redo( UICodeEditor *aEditor )
    {
        if( !mRemoved.empty() )
        {
    ...
    const UICodeEditor::LanguageDefinition &UICodeEditor::LanguageDefinition::CPlusPlus()
    {
		...
		static const char* const identifiers[] = {
            ...
            for( auto &k : identifiers )
            {
                ...
                langDef.mIdentifiers.insert( std::make_pair( string_t( k ), id ) );
            }
    ...
    const UICodeEditor::LanguageDefinition &UICodeEditor::LanguageDefinition::HLSL()
    {
        static bool               inited = false;
        static LanguageDefinition langDef;
        if( !inited )
		...
		static const char* const identifiers[] = {
            ...
            langDef.mTokenRegexStrings.push_back(
                std::make_pair<string_t, PaletteIndex>( "[ \\t]*#[ \\t]*[a-zA-Z_]+", PaletteIndex::Preprocessor ) );
            langDef.mTokenRegexStrings.push_back(
                std::make_pair<string_t, PaletteIndex>( "L?\\\"(\\\\.|[^\\\"])*\\\"", PaletteIndex::String ) );
            langDef.mTokenRegexStrings.push_back(
                std::make_pair<string_t, PaletteIndex>( "\\'\\\\?[^\\']\\'", PaletteIndex::CharLiteral ) );
            langDef.mTokenRegexStrings.push_back( std::make_pair<string_t, PaletteIndex>(
                "[+-]?([0-9]+([.][0-9]*)?|[.][0-9]+)([eE][+-]?[0-9]+)?[fF]?", PaletteIndex::Number ) );
            langDef.mTokenRegexStrings.push_back(
                std::make_pair<string_t, PaletteIndex>( "[+-]?[0-9]+[Uu]?[lL]?[lL]?", PaletteIndex::Number ) );
            langDef.mTokenRegexStrings.push_back(
                std::make_pair<string_t, PaletteIndex>( "0[0-7]+[Uu]?[lL]?[lL]?", PaletteIndex::Number ) );
            langDef.mTokenRegexStrings.push_back(
                std::make_pair<string_t, PaletteIndex>( "0[xX][0-9a-fA-F]+[uU]?[lL]?[lL]?", PaletteIndex::Number ) );
            langDef.mTokenRegexStrings.push_back(
                std::make_pair<string_t, PaletteIndex>( "[a-zA-Z_][a-zA-Z0-9_]*", PaletteIndex::Identifier ) );
            langDef.mTokenRegexStrings.push_back( std::make_pair<string_t, PaletteIndex>(
    ...
    const UICodeEditor::LanguageDefinition &UICodeEditor::LanguageDefinition::GLSL()
    {
        static bool               inited = false;
        static LanguageDefinition langDef;
        if( !inited )
    ...
    const UICodeEditor::LanguageDefinition &UICodeEditor::LanguageDefinition::C()
    {
        static bool               inited = false;
        static LanguageDefinition langDef;
        if( !inited )
    ...
    const UICodeEditor::LanguageDefinition &UICodeEditor::LanguageDefinition::SQL()
    {
        static bool               inited = false;
        static LanguageDefinition langDef;
        if( !inited )
		...
		static const char* const identifiers[] = {
            ...
            langDef.mTokenRegexStrings.push_back(
                std::make_pair<string_t, PaletteIndex>( "L?\\\"(\\\\.|[^\\\"])*\\\"", PaletteIndex::String ) );
            langDef.mTokenRegexStrings.push_back( std::make_pair<string_t, PaletteIndex>( "\\\'[^\\\']*\\\'", PaletteIndex::String ) );
            langDef.mTokenRegexStrings.push_back( std::make_pair<string_t, PaletteIndex>(
                "[+-]?([0-9]+([.][0-9]*)?|[.][0-9]+)([eE][+-]?[0-9]+)?[fF]?", PaletteIndex::Number ) );
            langDef.mTokenRegexStrings.push_back(
                std::make_pair<string_t, PaletteIndex>( "[+-]?[0-9]+[Uu]?[lL]?[lL]?", PaletteIndex::Number ) );
    ...
    const UICodeEditor::LanguageDefinition &UICodeEditor::LanguageDefinition::AngelScript()
    {
        static bool               inited = false;
        static LanguageDefinition langDef;
        if( !inited )
		...
		static const char* const identifiers[] = {
            ...
            langDef.mTokenRegexStrings.push_back(
                std::make_pair<string_t, PaletteIndex>( "L?\\\"(\\\\.|[^\\\"])*\\\"", PaletteIndex::String ) );
            langDef.mTokenRegexStrings.push_back(
                std::make_pair<string_t, PaletteIndex>( "\\'\\\\?[^\\']\\'", PaletteIndex::String ) );
            langDef.mTokenRegexStrings.push_back( std::make_pair<string_t, PaletteIndex>(
                "[+-]?([0-9]+([.][0-9]*)?|[.][0-9]+)([eE][+-]?[0-9]+)?[fF]?", PaletteIndex::Number ) );
            langDef.mTokenRegexStrings.push_back(
                std::make_pair<string_t, PaletteIndex>( "[+-]?[0-9]+[Uu]?[lL]?[lL]?", PaletteIndex::Number ) );
    ...
    const UICodeEditor::LanguageDefinition &UICodeEditor::LanguageDefinition::Lua()
    {
        static bool               inited = false;
        static LanguageDefinition langDef;
        if( !inited )
