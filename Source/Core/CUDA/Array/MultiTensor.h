...
namespace SE::Cuda
{
    ...
    struct sTensorShape
    {
        vec_t<vec_t<uint32_t>> mShape         = {}; //!< Shape
        vec_t<vec_t<uint32_t>> mStrides       = {}; //!< Strides
        ...
        vec_t<uint32_t>              mMaxDimensions = {}; //!< Pointwise maximum of the elements in the shape vector
        uint32_t                           mMaxBufferSize = 0;  //!< Size, in bytes, of the largest tensor.
        size_t                             mByteSize      = 0;  //!< Size, in bytes, of the entire tensor
        vec_t<sBufferSizeInfo>       mBufferSizes = {}; //!< Size and offsets information of each layer in the tensor, in bytes.
        ...
        ///
        sTensorShape( vec_t<vec_t<uint32_t>> const &aShape, size_t aElementSize );
        ...
        /// @param aElementSize Size, in bytes, of individual tensor elements.
        ///
        sTensorShape( vec_t<uint32_t> const &aShape, size_t aElementSize );
        ...
        vec_t<uint32_t> const &GetShapeForLayer( uint32_t i ) const
        {
            if( i >= CountLayers() )
        ...
        vec_t<uint32_t> const &GetStridesForLayer( uint32_t i ) const
        {
            if( i >= CountLayers() )
                throw std::out_of_range(
                    fmt::format( "Attempted to access layer {}, but the stack only has {} layers", i + 1, CountLayers() ) );
        ...
        ///
        vec_t<uint32_t> const GetDimension( int32_t i ) const;
        ...
        ///
        void InsertDimension( int32_t aPosition, vec_t<uint32_t> aDimension );
        ...
        vec_t<sBufferSizeInfo> GetTypedBufferSizes() const
        {
            vec_t<sBufferSizeInfo> lReturn( mBufferSizes.begin(), mBufferSizes.end() );
            for( auto &x : lReturn )
            {
    ...
    class MultiTensor
    {
      public:
        ...
        vec_t<_Tx> FetchBufferAt( uint32_t i ) const
        {
        ...
        ///
        template <typename _Tx>
        vec_t<_Tx> FetchFlattened() const
        {
        ...
        ///
        template <typename _Tx>
        void Upload( vec_t<_Tx> const &aArray ) const
        {
        ...
        ///
        template <typename _Tx>
        void Upload( vec_t<_Tx> const &aArray, uint32_t aLayer, uint32_t aOffset ) const
        {
        ...
        ///
        template <typename _Tx>
        void Upload( vec_t<_Tx> const &aArray, uint32_t aLayer ) const
        {
