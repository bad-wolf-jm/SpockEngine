...
namespace math
{
    ...
    template <typename T> vec3 make_vec3( T *V ) { return vec3(glm::make_vec3( V )); }
    template <typename T> vec4 make_vec4( T *V ) { return vec4(glm::make_vec4( V )); }
    template <typename T> mat4 make_mat3x3( T *V ) { return mat4(glm::make_mat3x3( V )); }
    template <typename T> mat4 make_mat4x4( T *V ) { return mat4(glm::make_mat4x4( V )); }
    template <typename T> quat make_quat( T *V ) { return quat(glm::make_quat( V )); }
