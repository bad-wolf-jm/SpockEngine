...
namespace LTSE::Graphics
{
    ...
    RenderContext::RenderContext( GraphicContext const &aGraphicContext, Ref<AbstractRenderTarget> aRenderTarget )
    ...
    void RenderContext::Present() { mRenderTarget->Present(); }

    void RenderContext::ResetBuffers()
    {
        mCurrentVertexBuffer = nullptr;
        mCurrentIndexBuffer  = nullptr;
    }
    ...
    void RenderContext::Bind( Ref<GraphicsPipeline> a_GraphicPipeline )
    {
        auto lCommandBuffer = mRenderTarget->GetCommandBuffer( mCurrentCommandBuffer );
    ...
    void RenderContext::Bind( Ref<Buffer> a_VertexBuffer, uint32_t a_BindPoint )
    {
        auto lCommandBuffer = mRenderTarget->GetCommandBuffer( mCurrentCommandBuffer );
    ...
    void RenderContext::Bind( Ref<Buffer> a_VertexBuffer, Ref<Buffer> a_IndexBuffer, uint32_t a_BindPoint )
    {
        auto lCommandBuffer = mRenderTarget->GetCommandBuffer( mCurrentCommandBuffer );
    ...
    void RenderContext::Bind( Ref<DescriptorSet> a_DescriptorSet, uint32_t a_SetIndex, int32_t a_DynamicOffset )
    {
        auto lCommandBuffer = mRenderTarget->GetCommandBuffer( mCurrentCommandBuffer );
