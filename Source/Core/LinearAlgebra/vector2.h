...
namespace numlua::linalg
{
    ...
    struct vect<2, _Ty>
    {
        ...
        // clang-format off
        LINALG_FUNCTION vect<2, _Ty>( _Ty _x, _Ty _y )
            : x{ _x } , y{ _y } { }

        LINALG_FUNCTION vect<2, _Ty>( _Ty _x )
            : x{ _x } , y{ _x } { }

        LINALG_FUNCTION vect<2, _Ty>( vect<2, _Ty> const &v )
            : x{ v.x } , y{ v.y } { }

        LINALG_FUNCTION vect<2, _Ty>( vect<3, _Ty> const &v )
            : x{ v.x } , y{ v.y } { }

        LINALG_FUNCTION vect<2, _Ty>( vect<4, _Ty> const &v )
            : x{ v.x } , y{ v.y } { }
        // clang-format on

        template <typename X, typename Y, typename Z, typename W>
        LINALG_FUNCTION vect<2, _Ty>( X _x, Y _y )
        ...
        LINALG_FUNCTION vect<2, _Ty> &operator=( vect<2, _Ty> const &v )
        {
        ...
        template <typename U>
        LINALG_FUNCTION vect<2, _Ty> &operator+=( U scalar )
        {
            return ( *this = detail::compute_vec_add<2, _Ty>::call( *this, vect<2, _Ty>( scalar ) ) );
        }

        template <typename U>
        LINALG_FUNCTION vect<2, _Ty> &operator+=( vect<2, U> const &v )
        {
    ...
    namespace detail
    {
        ...
        {
            LINALG_FUNCTION static vec<2, R> call( R ( *Func )( T x ), vec<2, T> const &v )
            {
        ...
        struct functor2<vec, 2, T>
        {
            ...
            template <class Fct>
            LINALG_FUNCTION static vec<2, T> call( Fct Func, vec<2, T> const &a, vec<2, T> const &b )
            {
                return vec<2, T>( Func( a.x, b.x ), Func( a.y, b.y ) );
            }
        };
