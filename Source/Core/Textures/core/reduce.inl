...
namespace numlua::core
{
    ...
    namespace detail
    {
        ...
        struct compute_sampler_reduce_1d
        {
            ...
            static vec<4, val_type> call( texture1d const &A, texture1d const &B, binary_func<val_type> TexelFunc,
        ...
        struct compute_sampler_reduce_1d_array
        {
            ...
            static vec<4, val_type> call( texture1d_array const &A, texture1d_array const &B, binary_func<val_type> TexelFunc,
                                          binary_func<val_type> ReduceFunc )
            {
                GLI_ASSERT( are_compatible( A, B ) );
        ...
        struct compute_sampler_reduce_2d
        {
            ...
            static vec<4, val_type> call( texture2d const &A, texture2d const &B, binary_func<val_type> TexelFunc,
                                          binary_func<val_type> ReduceFunc )
            {
                GLI_ASSERT( are_compatible( A, B ) );
        ...
        struct compute_sampler_reduce_2d_array
        {
            ...
            static vec<4, val_type> call( texture2d_array const &A, texture2d_array const &B, binary_func<val_type> TexelFunc,
                                          binary_func<val_type> ReduceFunc )
            {
                GLI_ASSERT( are_compatible( A, B ) );
                ...
                for( size_type LayerIndex = 0, LayerCount = A.layers(); LayerIndex < LayerCount; ++LayerIndex )
                    for( size_type LevelIndex = 0, LevelCount = A.levels(); LevelIndex < LevelCount; ++LevelIndex )
                    {
                        ...
                        for( TexelIndex.y = 0; TexelIndex.y < TexelCount.y; ++TexelIndex.y )
                            ...
                            {
                                Result =
                                    ReduceFunc( Result, TexelFunc( SamplerA.template fetch( TexelIndex, LayerIndex, LevelIndex ),
                                                                   SamplerB.template fetch( TexelIndex, LayerIndex, LevelIndex ) ) );
                            }
                    }
        ...
        struct compute_sampler_reduce_3d
        {
            ...
            static vec<4, val_type> call( texture3d const &A, texture3d const &B, binary_func<val_type> TexelFunc,
                                          binary_func<val_type> ReduceFunc )
            {
                GLI_ASSERT( are_compatible( A, B ) );
        ...
        struct compute_sampler_reduce_cube
        {
            ...
            static vec<4, val_type> call( texture_cube const &A, texture_cube const &B, binary_func<val_type> TexelFunc,
                                          binary_func<val_type> ReduceFunc )
            {
                GLI_ASSERT( are_compatible( A, B ) );
                ...
                for( size_type FaceIndex = 0, FaceCount = A.faces(); FaceIndex < FaceCount; ++FaceIndex )
                    for( size_type LevelIndex = 0, LevelCount = A.levels(); LevelIndex < LevelCount; ++LevelIndex )
                    {
                        ...
                        for( TexelIndex.y = 0; TexelIndex.y < TexelCount.y; ++TexelIndex.y )
                            ...
                            {
                                Result =
                                    ReduceFunc( Result, TexelFunc( SamplerA.template fetch( TexelIndex, FaceIndex, LevelIndex ),
        ...
        struct compute_sampler_reduce_cube_array
        {
            ...
            static vec<4, val_type> call( texture_cube_array const &A, texture_cube_array const &B, binary_func<val_type> TexelFunc,
                                          binary_func<val_type> ReduceFunc )
            {
                GLI_ASSERT( are_compatible( A, B ) );
                        ...
                        for( size_type LevelIndex = 0, LevelCount = A.levels(); LevelIndex < LevelCount; ++LevelIndex )
                        {
                            ...
                            for( TexelIndex.y = 0; TexelIndex.y < TexelCount.y; ++TexelIndex.y )
                                for( TexelIndex.x = 0; TexelIndex.x < TexelCount.x; ++TexelIndex.x )
                                {
                                    Result = ReduceFunc(
                                        Result,
                                        TexelFunc( SamplerA.template fetch( TexelIndex, LayerIndex, FaceIndex, LevelIndex ),
            ...
            static vec_type call( texture1d_array const &A, texture1d_array const &B, func_type TexelFunc, func_type ReduceFunc )
            {
                ...
                for( size_type LayerIndex = 0, LayerCount = A.layers(); LayerIndex < LayerCount; ++LayerIndex )
                    for( size_type LevelIndex = 0, LevelCount = A.levels(); LevelIndex < LevelCount; ++LevelIndex )
                    {
                        ...
                        {
                            Result =
                                ReduceFunc( Result, TexelFunc( A.template load<vec_type>( TexelIndex, LayerIndex, LevelIndex ),
                ...
                for( size_type FaceIndex = 0, FaceCount = A.faces(); FaceIndex < FaceCount; ++FaceIndex )
                    for( size_type LevelIndex = 0, LevelCount = A.levels(); LevelIndex < LevelCount; ++LevelIndex )
                    {
                        ...
                        for( TexelIndex.y = 0; TexelIndex.y < TexelCount.y; ++TexelIndex.y )
                            ...
                            {
                                Result =
                                    ReduceFunc( Result, TexelFunc( A.template load<vec_type>( TexelIndex, FaceIndex, LevelIndex ),
                        ...
                        for( size_type LevelIndex = 0, LevelCount = A.levels(); LevelIndex < LevelCount; ++LevelIndex )
                        {
                            ...
                            for( TexelIndex.y = 0; TexelIndex.y < TexelCount.y; ++TexelIndex.y )
                                for( TexelIndex.x = 0; TexelIndex.x < TexelCount.x; ++TexelIndex.x )
                                {
                                    Result = ReduceFunc(
                                        Result,
                                        TexelFunc( A.template load<vec_type>( TexelIndex, LayerIndex, FaceIndex, LevelIndex ),
    ...
    template <typename vec_type>
    inline vec_type reduce( texture1d const &In0, texture1d const &In1, typename reduce_func<vec_type>::type TexelFunc,
    ...
    template <typename vec_type>
    inline vec_type reduce( texture1d_array const &In0, texture1d_array const &In1, typename reduce_func<vec_type>::type TexelFunc,
                            typename reduce_func<vec_type>::type ReduceFunc )
    {
    ...
    template <typename vec_type>
    inline vec_type reduce( texture2d const &In0, texture2d const &In1, typename reduce_func<vec_type>::type TexelFunc,
                            typename reduce_func<vec_type>::type ReduceFunc )
    {
    ...
    template <typename vec_type>
    inline vec_type reduce( texture2d_array const &In0, texture2d_array const &In1, typename reduce_func<vec_type>::type TexelFunc,
                            typename reduce_func<vec_type>::type ReduceFunc )
    {
    ...
    template <typename vec_type>
    inline vec_type reduce( texture3d const &In0, texture3d const &In1, typename reduce_func<vec_type>::type TexelFunc,
                            typename reduce_func<vec_type>::type ReduceFunc )
    {
    ...
    template <typename vec_type>
    inline vec_type reduce( texture_cube const &In0, texture_cube const &In1, typename reduce_func<vec_type>::type TexelFunc,
                            typename reduce_func<vec_type>::type ReduceFunc )
    {
    ...
    template <typename vec_type>
    inline vec_type reduce( texture_cube_array const &In0, texture_cube_array const &In1,
                            typename reduce_func<vec_type>::type TexelFunc, typename reduce_func<vec_type>::type ReduceFunc )
    {
