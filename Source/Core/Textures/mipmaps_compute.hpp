...
namespace numlua::core
{
    namespace detail
    {
                                         ...
                                         filter Min )
        {
            ...
            typedef typename detail::filterBase<detail::DIMENSION_1D, texture_type, interpolate_type, normalized_type, fetch_func,
                                                texel_type>::filterFunc    filter_func;

            filter_func const Filter =
                detail::get_filter<filter_func, detail::DIMENSION_1D, texture_type, interpolate_type, normalized_type, fetch_func,
                                   texel_type, sampler_value_type>( FILTER_NEAREST, Min, false );
            GLI_ASSERT( Filter );
                ...
                for( size_type Face = BaseFace; Face <= MaxFace; ++Face )
                    for( size_type Level = BaseLevel; Level < MaxLevel; ++Level )
                    {
                        ...
                        for( component_type i = 0; i < ExtentDst.x; ++i )
                        {
                            ...
                            texel_type const &Texel = Filter( Texture, Fetch, SamplePosition, Layer, Face,
                                         ...
                                         filter Min )
        {
            ...
            typedef typename texture_type::size_type                       size_type;
            typedef typename extent_type::value_type                       component_type;
            typedef typename detail::filterBase<detail::DIMENSION_2D, texture_type, interpolate_type, normalized_type, fetch_func,
                                                texel_type>::filterFunc    filter_func;

            filter_func const Filter =
                detail::get_filter<filter_func, detail::DIMENSION_2D, texture_type, interpolate_type, normalized_type, fetch_func,
                                   texel_type, sampler_value_type>( FILTER_NEAREST, Min, false );
            GLI_ASSERT( Filter );
                                         ...
                                         filter Min )
        {
            ...
            typedef typename texture_type::size_type                       size_type;
            typedef typename extent_type::value_type                       component_type;
            typedef typename detail::filterBase<detail::DIMENSION_3D, texture_type, interpolate_type, normalized_type, fetch_func,
                                                texel_type>::filterFunc    filter_func;

            filter_func const Filter =
                detail::get_filter<filter_func, detail::DIMENSION_3D, texture_type, interpolate_type, normalized_type, fetch_func,
                                   texel_type, sampler_value_type>( FILTER_NEAREST, Min, false );
            GLI_ASSERT( Filter );
