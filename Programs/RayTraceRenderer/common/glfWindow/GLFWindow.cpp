...
namespace osc
{
    ...
    static void glfw_error_callback( int error, const char *description ) { fprintf( stderr, "Error: %s\n", description ); }
    ...
    static void glfwindow_reshape_cb( GLFWwindow *window, int width, int height )
    {
        ...
        assert( gw );
        gw->resize( math::ivec2( width, height ) );
        // assert(GLFWindow::current);
        //   GLFWindow::current->resize(math::ivec2(width,height));
    }
    ...
    static void glfwindow_mouseMotion_cb( GLFWwindow *window, double x, double y )
    {
        GLFWindow *gw = static_cast<GLFWindow *>( glfwGetWindowUserPointer( window ) );
        assert( gw );
        gw->mouseMotion( math::ivec2( (int)x, (int)y ) );
    }
    ...
    void GLFWindow::run()
    {
        ...
        resize( math::ivec2( width, height ) );
