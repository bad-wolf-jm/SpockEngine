...
namespace gdt
{
    ...
    struct interval
    {
        ...
        inline __both__ interval( T begin, T end )
            : begin( begin )
            , end( end )
        {
        }
        ...
        static inline __both__ interval<T> positive() { return interval<T>( 0.f, gdt::open_range_upper<T>() ); }
    };
    ...
    struct box_t
    {
        ...
        inline __both__ box_t including( const vec_t &other ) const { return box_t( min( lower, other ), max( upper, other ) ); }
        ...
        inline __both__ interval<scalar_t> get_slab( const uint32_t dim ) { return interval<scalar_t>( lower[dim], upper[dim] ); }
        ...
        inline __both__ vec_t center() const { return ( lower + upper ) / (typename vec_t::value_type)2; }
        inline __both__ vec_t span() const { return upper - lower; }
        inline __both__ vec_t size() const { return upper - lower; }

        inline __both__ typename long_type_of<typename T::value_type>::type volume() const { return gdt::volume( size() ); }
