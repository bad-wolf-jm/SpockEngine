...
#define ScalarT typename L::vector_t::value_type
    ...
    struct GDT_INTERFACE AffineSpaceT
    {
        ...
        inline AffineSpaceT( const VectorT &vx, const VectorT &vy, const VectorT &vz, const VectorT &p )
            : l( vx, vy, vz )
            , p( p )
        {
        }
        inline AffineSpaceT( const L &l, const VectorT &p )
            : l( l )
            , p( p )
        {
        }
        ...
        inline AffineSpaceT( const AffineSpaceT<L1> &s )
            : l( s.l )
            , p( s.p )
        {
        }
        ...
        inline AffineSpaceT( ZeroTy )
            : l( zero )
            , p( zero )
        {
        }
        inline AffineSpaceT( OneTy )
            : l( one )
            , p( zero )
        {
        }
    ...
    template <typename L>
    inline AffineSpaceT<L> &operator*=( AffineSpaceT<L> &a, const AffineSpaceT<L> &b )
    {
        return a = a * b;
    }
    template <typename L>
    ...
    ////////////////////////////////////////////////////////////////////////////////
    /*! Template Specialization for 2D: return matrix for rotation around point (rotation around arbitrarty vector is not meaningful in
     * 2D) */
    template <>
    inline AffineSpace2f AffineSpace2f::rotate( const math::vec2 &p, const float &r )
    {
