...
LAYOUT_LOCATION( 0 ) INOUT float outNear; // 0.01
LAYOUT_LOCATION( 1 ) INOUT float outFar;  // 100
LAYOUT_LOCATION( 2 ) INOUT float3 outNearPoint;
LAYOUT_LOCATION( 3 ) INOUT float3 outFarPoint;
LAYOUT_LOCATION( 4 ) INOUT float4x4 outFragView;
LAYOUT_LOCATION( 8 ) INOUT float4x4 outFragProj;
...
float computeLinearDepth( float3 pos )
{
    ...
    float linearDepth =
        ( 2.0 * near * far ) / ( far + near - clip_space_depth * ( far - near ) ); // get linear value between 0.01 and 100
...
void main()
{
    ...
    outNear      = 0.01;
    outFar       = 25.0f;
    outFragView  = gView.mViewMatrix;
    outFragProj  = gView.mProjectionMatrix;
    outNearPoint = UnprojectPoint( p.x, p.y, 0.0, gView.mViewMatrix, gView.mProjectionMatrix ).xyz; // unprojecting on the inNear plane
    outFarPoint  = UnprojectPoint( p.x, p.y, 1.0, gView.mViewMatrix, gView.mProjectionMatrix ).xyz; // unprojecting on the inFar plane
